...
function specification() {
    ...
    for(var loop=0; loop<raw_data.length; loop++)
    {
      if(raw_data.substring(loop, loop+6)== "Label:") {
        ...
        if(raw_data.substring(loop, loop+4).toLowerCase()== "age:") {
          for(var loop1 = loop+6; loop1<raw_data.length; loop1++) {
            if(raw_data.substring(loop1, loop1+1)=="\n")
              break;
        ...
        if((temp_release_date.substring(temp_release_date.length-4,temp_release_date.length).trim())>
          date.getFullYear())
          alert("Release year is greater than 2019");
        }
      //       else
      //   if(raw_data.substring(loop, loop+13)== "Release Date:") {
      ...
      // }
          ...
          raw_data.substring(loop, loop+5).toLowerCase()== "rank:")
        {
          if(raw_data.substring(loop, loop+9).toLowerCase()== "add date:")
            alert("Product should not be created as 'Add Date' is detected on Webami");
...
function dataCollection() {
  ...
  for(var i = 0; i < html_data.length; i++) {
    ...
    if(html_data.substring(i, i+43) == "ProductFeatures__BulletText-s6my6ry-2 UbHkR") {
        for(var i1 = i+43; i1 < html_data.length; i1++) {
        ...
        track_value += "\n\t\t\t\t\t\t"
          break;
                        ...
                        //   track_value += "\n";
                        //     track_value += "\t\t\t\t\t<li>";
                            if(html_data.substring(i1, i1+4) == "<li>"){
                        ...
                        for(var i2 = i1; i2 < html_data.length; i2++) {
                            ...
                            track_value += html_data.substring(i2, i2+1) ;
    ...
    if(html_data.substring(i, i+29) == 
      "product-details__ages-wrapper") {
      lbl:{
        alert("product-details__ages-wrapper");
      for(var i0 = i; i0 < html_data.length; i0++) {
          if(html_data.substring(i0, i0+72) == 
      'ProductDetails__Label-s16q3dpl-2 bsbgZz Text__BaseText-aa2o0i-0 daDMSr">'){
        alert("Label-s16q3dpl-2");
       
          for(var i1 = i0+72; i1 < html_data.length; i1++) {
         
            if(html_data.substring(i1, i1+46) == '<button aria-label="Facebook" label="Facebook"') {
              i = i1;
              alert("Facebook");
              break lbl;
            }
            if(html_data.substring(i1, i1+5) == '</dd>'||
              html_data.substring(i1, i1+12) == '</span></dd>') {
              spec_value += "\n";
               alert("</span></dd>");
              break;
            }
            if(html_data.substring(i1, i1+5) == '</dt>') {
                alert("</dt>");
              spec_value +=": ";
              for(var i2 = i1; i2 < html_data.length; i2++) {
    ...
    if(html_data.substring(i, i+32) == "product-details-long-description") {
      for(var i0 = i+24; i0 < html_data.length; i0++) {
     
        if(html_data.substring(i0-6, i0) == '<span>') {
             
          for(var i1 = i0; i1 < html_data.length; i1++) {
         
            if(html_data.substring(i1, i1+7) == '</span>') {
              i = i1;
              break;
            }
    ...
    if(html_data.substring(i, i+35) == "Accordion__Content-s34qc4s-1 gPQwoT") {
      for(var i0 = i+35; i0 < html_data.length; i0++) {
     
        if(html_data.substring(i0-9, i0) == '<span><p>') {
          if(upper!="")
            upper+="<br />"
          for(var i1 = i0; i1 < html_data.length; i1++) {
         
            if(html_data.substring(i1, i1+11) == '</p></span>') {
              i = i1;
              break;
            }
