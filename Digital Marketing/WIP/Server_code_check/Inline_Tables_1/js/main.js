$(function(){
       ...
       lbl: {
          ...
          if(c_status_option_checked[c_status_option_checked_count]=="Text Field")
            ...
            update_data='else\nif(ctc[temp_i].toLowerCase().trim()==\n"'+
              ...
              +'")\nctc[temp_i]="'+
              data_store_ctc[c_status_option_checked_count][1].toLowerCase().trim()+'";';
  ...
  function inlineTableRead(index) {
      ...
      // alert("tab_count 0");
            ...
            if((sheet_data.substring(i,i+1)) == "\t") {
              ...
              if(title[row_count].indexOf('"')==0)
                title[row_count]=title[row_count].slice(1, title[row_count].length);
    ...
    function updatedCtc(temp_i, temp_i2){
          ...
          if(ctc[temp_i].toLowerCase().trim()=="upper & lower description created, title updated, gallery image added")
          ctc[temp_i]="created and uploaded";

          else
          if(ctc[temp_i].toLowerCase().trim()=="upper & lower description created, title updated, gallery image enhanced")
          ctc[temp_i]="created and uploaded";

          else
          if(ctc[temp_i].toLowerCase().trim()=="upper and lower description created, images added and title updated")
          ctc[temp_i]="created and uploaded";

          else
          if(ctc[temp_i].toLowerCase().trim()=="upper description created, title updated, gallery image added")
          ctc[temp_i]="created and uploaded";
        else
          if(ctc[temp_i].toLowerCase().trim()==
          "query regarding review manufacturer url")
    ...
    function inlineTableWriteClient() {
        ...
        for(var i1=0;i1<date_checked.length;i1++) {
          ...
          if(s_status[i].toLowerCase().trim()=="uploaded"){
            ...
            "\t\t"+gio_f[i]+"\t"+gi_enhanced[i]+"\t"+giu_date[i]+"\n";
      ...
      "\tGallery Images On Frys\tGallery images Enhanced\tGallery Images Downloaded\tTitle Editable\n"+
      creation_data_editable+"\n\n";
      ...
      if(aof_data!="")
        text_inline_output+=
      "Plu\tDescription\tCreation Status\tSpecification Status\tFry's Url\t"+
      "\tGallery Images On Frys\tGallery images Enhanced\tGallery Images Downloaded\n"+
      aof_data+"\n\n";
      if(pnf_data!="")
    ...
    var audit_data=auditSheet();
      ...
      "\nPNF\t"+pnf_count+"\nSpec-Tested\t"+spec_tested_count+"\nSpec Uploaded\t"+spec_uploaded_count+
      "\nSpec Hyphen\t"+spec_tested_hyphen_count+audit_data;
    ...
    function auditSheet() {
      ...
      var audit_data="\n\nAudit Sheet\nTask Deliver\t"+audit_plu_count+"\nTested\t"+
      audit_test_count+"\nCreated/Uploaded\t"+audit_create_count;
